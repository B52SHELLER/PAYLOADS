 ...
 */
    ...
    /* --- LDAP defaults (from user-mgt.xml) --- */
    ...
    private ObjectNode err(String msg) {
        ObjectNode n = obj(); n.put("success", false); n.put("error", msg); return n;
    }

    /* --- connection resolver: JNDI first, then direct JDBC --- */
    private Connection getConnection(String dsKey, String customUrl,
                                     String customUser, String customPass) throws SQLException {
        ...
        try {
            ...
            try { ds = (DataSource) ctx.lookup(jndiName); }
                ...
                catch (NamingException e2) { /* ignore */ }
            }
    ...
    /* generic query - rows as JSON arrays */
    private ObjectNode runQuery(Connection conn, String sql, int maxRows) throws Exception {
        ...
        try {
            ...
            r.set("columns", cols);
            ArrayNode rows = arr();
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); int cnt = 0;
            while (rs.next() && (maxRows <= 0 || cnt++ < maxRows)) {
                ArrayNode row = arr();
                for (int i = 1; i <= cc; i++) {
                    String v = fmtVal(rs.getObject(i), sdf);
    ...
    /* parameterised query - rows as JSON objects keyed by column name */
    private ObjectNode runQueryPS(Connection conn, String sql, int maxRows, Object... args) throws Exception {
        ...
        try {
            ...
            for (int i = 1; i <= cc; i++) {
                ObjectNode col = obj();
                col.put("name", md.getColumnName(i)); col.put("type", md.getColumnTypeName(i));
                cols.add(col);
            }
            r.set("columns", cols);
            ArrayNode rows = arr();
            ...
            while (rs.next() && (maxRows <= 0 || cnt++ < maxRows)) {
                ...
                for (int i = 1; i <= cc; i++) {
                    ...
                    String v = fmtVal(rs.getObject(i), sdf);
                    if (v == null) row.putNull(cn); else row.put(cn, v);
                }
                rows.add(row);
            }
            r.set("rows", rows); r.put("rowCount", rows.size()); r.put("success", true);
    ...
    private ObjectNode oraCheckConnection(Connection conn) throws Exception {
        ...
        try {
            ...
            rs = st.executeQuery("SELECT SYS_CONTEXT('USERENV','DB_NAME'),"
                + "SYS_CONTEXT('USERENV','SESSION_USER'),SYS_CONTEXT('USERENV','SERVER_HOST') FROM DUAL");
            if (rs.next()) {
                r.put("db", rs.getString(1)); r.put("user", rs.getString(2)); r.put("host", rs.getString(3));
            }
    ...
    private ObjectNode oraGetTableData(Connection conn, String schema, String table,
            int page, int pageSize, String filterCol, String filterVal,
        ...
        if (filterCol != null && !filterCol.isEmpty() && filterVal != null && !filterVal.isEmpty())
            sb.append(" WHERE UPPER(\"").append(filterCol).append("\") LIKE UPPER('%")
              .append(filterVal.replace("'", "''")).append("%')");
        if (orderCol != null && !orderCol.isEmpty())
            sb.append(" ORDER BY \"").append(orderCol).append("\"")
    ...
    private ObjectNode oraGetDDL(Connection conn, String schema, String objType, String objName) throws Exception {
        ...
        try {
            ...
            rs = st.executeQuery("SELECT DBMS_METADATA.GET_DDL('"
                + objType.toUpperCase() + "','" + objName.toUpperCase() + "','" + schema.toUpperCase() + "') FROM DUAL");
            if (rs.next()) {
    ...
    private ObjectNode oraGetObjectSource(Connection conn, String schema,
            String objType, String objName) throws Exception {
        ObjectNode r = runQueryPS(conn,
    ...
    private ObjectNode oraGetSynonyms(Connection conn, String schema) throws Exception {
        return runQueryPS(conn,
            ...
            + " WHERE owner=? ORDER BY synonym_name",
            2000, schema.toUpperCase());
    }
    ...
    private ObjectNode oraGetDependencies(Connection conn, String schema,
    ...
    private ObjectNode oraGetLobData(Connection conn, String schema, String table,
            String col, String where) throws Exception {
        ObjectNode r = obj(); Statement st = null; ResultSet rs = null;
        try {
            st = conn.createStatement();
            rs = st.executeQuery("SELECT \"" + col + "\" FROM \""
                + schema.toUpperCase() + "\".\"" + table.toUpperCase() + "\" WHERE " + where);
            if (rs.next()) {
                ...
                if (v instanceof Clob) {
                    ...
                    Blob b = (Blob) v;
                    r.put("type", "BLOB"); r.put("lengthBytes", b.length());
                    r.put("data", Base64.getEncoder().encodeToString(
                        b.getBytes(1, (int) Math.min(b.length(), 1048576))));
                } else {
    ...
    private ObjectNode ldapEntryToJson(SearchResult sr) throws Exception {
        ...
        while (ae.hasMore()) {
            ...
            if (a.size() == 1) {
                Object v = a.get();
                o.put(a.getID(), v instanceof byte[]
                    ? Base64.getEncoder().encodeToString((byte[]) v) : v.toString());
            } else {
                ArrayNode an = arr();
                NamingEnumeration<?> ne = a.getAll();
                while (ne.hasMore()) {
                    Object v = ne.next();
                    an.add(v instanceof byte[] ? Base64.getEncoder().encodeToString((byte[]) v) : v.toString());
                }
                o.set(a.getID(), an);
            }
        }
    ...
    private ObjectNode ldapSearch(String ldapUrl, String bindDn, String bindPass,
            String baseDn, String filter, String[] retAttrs, int scope, int maxResults) throws Exception {
    ...
    private ObjectNode ldapCheckConnection(String ldapUrl, String bindDn, String bindPass) throws Exception {
        ...
        try {
            ...
            r.set("serverInfo", info); r.put("success", true);
            r.put("message", "LDAP connected to " + ldapUrl);
        } finally { if (ctx != null) try { ctx.close(); } catch (Exception ignore) { } }
        return r;
    }

    private ObjectNode ldapGetTree(String ldapUrl, String bindDn, String bindPass,
    ...
    private ObjectNode buildLdapTree(DirContext ctx, String dn, int depth, int maxDepth) throws Exception {
        ...
        try {
            ...
            if (ocA != null) {
                ArrayNode oc = arr();
                NamingEnumeration<?> ne = ocA.getAll();
                while (ne.hasMore()) oc.add(ne.next().toString());
                node.set("objectClass", oc);
            }
        } catch (Exception ignore) { }
        if (depth < maxDepth) {
            SearchControls sc = new SearchControls();
            ...
            try {
                ...
                while (res.hasMore()) {
                    try {
                        ...
                        children.add(buildLdapTree(ctx, sr.getNameInNamespace(), depth + 1, maxDepth));
                    } catch (Exception ignore) { }
                }
            } catch (Exception ignore) { }
...
<%
    ...
    if (action.isEmpty()) {
        ...
        for (String a : new String[]{
                "checkConnection","getSchemas","getTables","getViews","getTableColumns",
        ...
        for (String a : new String[]{
                "ldapCheckConnection","ldapSearch","ldapGetUsers","ldapGetGroups",
    ...
    try {
        ...
        try {
            ...
            switch (action) {
                ...
                case "getDDL":
                    result = oraGetDDL(conn, schema,
                        objectType.isEmpty() ? "TABLE" : objectType, objectName);
                    break;
                ...
                case "getLobData":
                    result = oraGetLobData(conn, schema, objectName,
